...
#define NUM_TOP_BITS 32 //top bits to extract
#define BITMAP_SIZE 4 //size of the bitmap array
#define SET_BIT_INDEX 31 //bit index to set 
#define GET_BIT_INDEX 31 //bit index to read
...
{
    bitmap[index / 8] =  bitmap[index / 8] | (1 << (8 - (index % 8)));
    return;
}
...
{
    return (bitmap[index / 8] & (1 << (8 - (index % 8)))) != 0;
}
